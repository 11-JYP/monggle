...
저는 **몽글로드 페이지에서 카테고리 기반 장소 검색 기능**과 **지도 연동 UI 구현**을 담당하였습니다.

### ✅ 주요 기여 영역
- Kakao Map API를 활용한 지역 기반 장소 검색 구현
- 사용자가 선택한 카테고리에 따라 지도에 마커 및 위치 정보 표시
- 검색 조건에 따른 장소 리스트 필터링 기능 개발
- 지도 중심 좌표 이동 및 반응형 UI 레이아웃 조정
